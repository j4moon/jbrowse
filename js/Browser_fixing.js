...
var Browser = function(params) {
    ...
    dojo.addOnLoad(
	    ...
	    brwsr.linkFullview(fullview_div);	    
...
/**
 * @returns {String} 0/1 for show_nav value
 * (passing to URL)
 */

Browser.prototype.visibleNav = function() {
     if ( this.show_nav == 0) return "0";
     else return "1"
};

/**
 * @returns {String} 0/1 for show_tracklist value
 * (passing to URL)
 */

Browser.prototype.visibleTracklist = function() {
     if ( this.show_tracklist == 0) return "0";
     else return "1";
};

Browser.prototype.linkFullview = function(area) {
    var fullview = document.createElement("a");
    fullview.appendChild(document.createTextNode("Fullview"));
    fullview.href = window.location.host + window.location.pathname;
    fullview.style.cssText = "float: right; clear";
    area.appendChild(fullview);
}

Browser.prototype.bkmrk = function(area) {
  //  if (this.bookmark) { //should be this.bookmark not params.bookmark
     var brwsr = this;
        this.link = document.createElement("a");
        this.link.appendChild(document.createTextNode("Link"));
        this.link.href = window.location.href;
        dojo.connect(this, "onCoarseMove", function() {
                         brwsr.link.href = this.params.bookmark(brwsr);
                     });
        dojo.connect(this, "onVisibleTracksChanged", function() {
                         brwsr.link.href = this.bookmark(brwsr);
                     });
        this.link.style.cssText = "float: right; clear; margin-right: 10px";
